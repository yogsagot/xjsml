...
function include(tag, args) {
    ...
    parsed.children.forEach(child => {
        if (child.action != 'closetag' && child.tag == 'param') {
            if (!child.args.hasOwnProperty("name")) {
                var e = new Error(`Cannot include template, file ${newfilename} has "param" tag with no "name" property defined`);
                e.fileName = tag.filename;
                e.lineNumber = tag.line;
                throw e;
            }
            var paramname = child.args.name.stringvalue;
            if (tag.args.hasOwnProperty(paramname)){
                var value = this.attrValue(tag.args[paramname], args)
                if (value != undefined)
    ...
    var newargs = {...args, ...params};
    return this.renderTag(parsed, newargs);
}
