...
function assign(tag, args, context) {
    ...
    var name = tag.args.name ? this.attrValue(tag.args.name, args) : this.attrValue(tag.args.var, args);
    var value = tag.args.value ? this.attrValue(tag.args.value, args) : this.attrValue(tag.args.val, args);
...
function withblock(tag, args, context) {
    var obj = this.attrValue(tag.args.args[0], args);
    if (typeof obj !== 'object')
        this.error(tag, `With tag must accept an an object as a parameter. Got ${typeof obj} instead`);
    var localargs = {
        ...args,
        ...obj
    };
