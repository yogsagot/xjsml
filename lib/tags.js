...
function include(tag, args, context) {
    ...
    tag.children.forEach(child => {
        if (child.tag == 'block' && child.action != 'closetag') {
            if (!child.args.name) {
                var e = new Error(`Block tag must have "name" property`);
                e.fileName = tag.filename;
                e.lineNumber = tag.line;
                throw e;
            }
    ...
    if (context != null && context.hasOwnProperty('blocks') && context.blocks.hasOwnProperty(name)) {
        ...
        if (context.blocks[name].args.mode) {
            ...
            if (!['replace', 'precede', 'append'].includes(mode)) {
                var e = new Error(`Block tag "mode" property must be "replace", "precede" or "append"`);
                e.fileName = context.blocks[name].filename;
                e.lineNumber = context.blocks[name].line;
                throw e;
            }
        }
...
function register(tag, args, context) {
    if (!tag.args.name) {
        var e = new Error(`Register tag must have "name" property`);
        e.fileName = tag.filename;
        e.lineNumber = tag.line;
        throw e;
    }
    ...
    if (!tag.args.src) {
        var e = new Error(`Register tag must have "src" property`);
        e.fileName = tag.filename;
        e.lineNumber = tag.line;
        throw e;
    }
    ...
    //     var e = new Error(`Tag with name "${name}" already registered`);
    //     e.fileName = tag.filename;
    //     e.lineNumber = tag.line;
    //     throw e;
    // }
    ...
    if (!fs.existsSync(newfilename)) {
        var e = new Error(`Cannot register tag ${name}, file ${newfilename} does not exist`);
        e.fileName = tag.filename;
        e.lineNumber = tag.line;
        throw e;
    }
...
function registered(tag, args, context) {
    if (context == null && !context.hasOwnProperty('registered') && !context.registered.hasOwnProperty(tag.tag)) {
        var e = new Error(`Tag "${tag.tag}" has not been regisered`);
        e.fileName = tag.filename;
        e.lineNumber = tag.line;
        throw e;
    }
