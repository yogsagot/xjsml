...
function replace(value, param) {
    if(param == null || param == undefined) {
        var e = new Error(`Attribute ${this.name} should have a parameter \n in file ${this.filename} on line ${this.line}`);
    ...
    if (Array.isArray(param)) {
        if(param.length != 2) {
            var e = new Error(`"search" modifier for attribute ${this.name} should have an array of two elements as a parameter \n in file ${this.filename} on line ${this.line}`);
            e.fileName = this.filename;
            e.lineNumber = this.line;
            throw e;
        }
    ...
    if(typeof param === 'object' && param.constructor === Object) {
        if (!param.hasOwnProperty("search")) {
            var e = new Error(`"search" modifier for attribute ${this.name} should have property "search" in a parameter \n in file ${this.filename} on line ${this.line}`);
            e.fileName = this.filename;
            e.lineNumber = this.line;
            throw e;
        }
        if (!param.hasOwnProperty("replace")) {
            var e = new Error(`"search" modifier for attribute ${this.name} should have property "replace" in a parameter \n in file ${this.filename} on line ${this.line}`);
            e.fileName = this.filename;
            e.lineNumber = this.line;
            throw e;
        }
