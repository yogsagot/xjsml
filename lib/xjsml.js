...
function renderTag(tag, args) {
    ...
    if (tag.action == 'html') {
        tagstr += renderTag(tag.children, args);
        return tagstr;
    }
    if (tag.action == 'write') {
        tagstr += tag.content;
    ...
    if (tag.action == 'opentag' || tag.action == 'selfclosetag') {
        if (registeredTags.hasOwnProperty(tag.tag)) {
            tagstr += registeredTags[tag.tag].call({renderTag: renderTag, attrValue: attrValue}, tag, args);
        } else {
            if (tag.action == 'opentag') {
...
function attrValue(attr, values) {
    ...
    try {
        ...
        throw `cannot parse argument ${attr.name} on line ${attr.line}: ${error}`;
    }
    attr.modifiers.forEach(mod => {
        ...
        if (mod.value != null) {
            try {
                ...
                throw `Cannot parse modifier ${mod.name} parameter on line ${attr.line}: ${error}`;
            }
        }
...
<p class=hello uppercase id="fffdf" uppercase=true lowercase>
<_ hello1 uppercase />
...
<if (num==45) >
    num is equal to 45
    <elseif (num == 44) />
    well no, it is <b>44</b>
    <div>
        <span id="vvv" uppercase><_ num /></span>
    </div>
    <else />
    num is something other
</if>
<_ hello1 />
<= hello1 />
...
<switch num>
    ...
    <case 43>
        well, we see the <b>right</b> case <_ (num + 20) />
    </case>
    <case 44>
        but what if it's actually 44?
    </case>
    <else>
        nothing works =(
    </else>
...
<for src=[{'a': 5}, {'a': 55}] item="value1">
 the key is <_ key /> and value is <_ value1.a />
</for>
...
var args = {
    hello: "fffdfdfdfd",
    hello1: "this is a variable <b>with tags</b>",
    num: 73
};
var parsed = parse(template2);
var str = renderTag(parsed.children, args);
console.log(str);
//console.log(parsed);
